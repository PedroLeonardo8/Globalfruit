			...
			Else
				...
				While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
					...
					If lContinua
						...
						If Empty((cAliasSD2)->D2_LOTECTL) //Rafael França e Rafael Bedendo - 09/02/2021 - Coloca o numero do lote depois da descrição do produto, apenas quando o lote é informado
						cDescProd := IIF(Empty(SC6->C6_DESCRI),SB1->B1_DESC,SC6->C6_DESCRI)
						Else
						cDescProd := IIF(Empty(SC6->C6_DESCRI),AllTrim(SB1->B1_DESC),AllTrim(SC6->C6_DESCRI))+" -Lote: "+AllTrim((cAliasSD2)->D2_LOTECTL)
						EndIf
						...
						If !Empty((cAliasSD2)->D2_IDENTB6) .And. lNFPTER
				         	If SC5->C5_TIPO == "N"
						         ...
						         If SA7->(MsSeek( xFilial("SA7") + (cAliasSD2)->(D2_CLIENTE+D2_LOJA+D2_COD) )) .and. !empty(SA7->A7_CODCLI) .and. !empty(SA7->A7_DESCCLI)
									...
									Else
									cDescProd := Alltrim(SA7->A7_DESCCLI) +" -Lote: "+AllTrim((cAliasSD2)->D2_LOTECTL)
									Endif
						         EndIf
							ElseIf SC5->C5_TIPO == "B"
						         ...
						         If SA5->(MsSeek( xFilial("SA5") + (cAliasSD2)->(D2_CLIENTE+D2_LOJA+D2_COD) )) .and. !empty(SA5->A5_CODPRF) .and. !empty(SA5->A5_DESREF)
									...
									Else
									cDescProd := Alltrim(SA5->A5_DESREF) +" -Lote: "+AllTrim((cAliasSD2)->D2_LOTECTL)
									Endif
						         EndIf
					      	EndIf
			         	EndIf
